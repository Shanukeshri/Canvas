    ...
    >
      ...
      >
        ...
        <header className="px-6 py-5 border-b border-surface-variant/30 flex items-center justify-between bg-surface-container-lowest/80 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-outline">Atmospheric layers for deep focus and flow</p>
            </div>
          </div>
          <button
            onClick={closeOverlay}
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {[
                ...
                <button
                  ...
                  className="px-3 py-2 rounded-xl bg-surface-container-low/80 hover:bg-surface-container border border-surface-variant/40 hover:border-primary/40 text-xs font-medium text-on-surface flex items-center gap-2 transition-all group"
                >
          ...
          {/* Current Live Mix Equalizer */}
          <div className="p-4 rounded-2xl bg-surface-container-low/60 border border-surface-variant/40 space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold text-on-surface flex items-center gap-2">
                <Sliders className="w-3.5 h-3.5" style={{ color: theme.hex }} />
                Active Atmosphere Mix
              </span>
              <span className="text-[11px] text-outline font-mono">
                {activeSounds.length === 0 || isMasterMuted ? 'Muted' : `${activeSounds.length} layers`}
              </span>
            </div>

            {activeSounds.length === 0 || isMasterMuted ? (
              <div className="py-2 text-xs text-outline italic flex items-center gap-2">
                <Sparkles className="w-3.5 h-3.5 text-outline/60" />
                No sounds currently playing. Tap play on any soundscape below to begin.
              </div>
            ) : (
              <div className="space-y-2">
                {activeSounds.map((sound) => (
                  <div key={sound.id} className="flex items-center gap-3">
                    <div className="text-primary shrink-0" style={{ color: theme.hex }}>
                      {getSoundIcon(sound.type, sound.id)}
                    </div>
                    <span className="text-xs font-medium text-on-surface w-28 truncate">
                      {sound.name}
                    </span>
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className={clsx(
                        'w-8 h-8 rounded-xl flex items-center justify-center transition-transform active:scale-95 shadow-sm',
                        isPlaying
                          ? 'text-white hover:opacity-90'
                          : 'bg-surface-container text-outline hover:text-on-surface hover:bg-surface-container-high border border-surface-variant/40'
                      )}
                  ...
                  <div className="flex items-center gap-2 pt-1">
                    <Volume2 className="w-3 h-3 text-outline shrink-0" />
                    <input
                      type="range"
                      min="0"
                      max="100"
        ...
        {/* Footer: Master Volume & Mute */}
        <footer className="px-6 py-4 border-t border-surface-variant/30 bg-surface-container-lowest/90 shrink-0 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 flex-1">
            <button
              onClick={toggleMasterMute}
              className="p-2 rounded-xl text-outline hover:text-on-surface hover:bg-surface-container transition-colors"
              title={isMasterMuted ? 'Unmute master audio' : 'Mute all audio'}
            >
              {isMasterMuted ? (
                <VolumeX className="w-5 h-5 text-error" />
              ) : (
                <Volume2 className="w-5 h-5" style={{ color: theme.hex }} />
              )}
            </button>
            <div className="flex-1 flex items-center gap-2">
              <input
                ...
                style={{ accentColor: theme.hex }}
              />
              <span className="text-xs font-mono text-outline w-12 text-right">
